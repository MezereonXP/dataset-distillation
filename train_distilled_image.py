...
class Trainer(object):
    ...
    def init_data_optim(self):
        ...
        # labels
        self.labels = []
        distill_label = torch.randn(state.distilled_images_per_class_per_step, state.num_classes, dtype=torch.float, device=state.device, requires_grad=True) \
    ...
    def forward(self, model, rdata, rlabel, steps):
        ...
        for step_i, (data, label, lr) in enumerate(steps):
            with torch.enable_grad():
                ...
                loss = task_loss(state, output.float(), label.float())
